...
def csr_to_user_dict(train_matrix):
    ...
    for idx, value in enumerate(train_matrix):
        if len(value.indices) > 0:
            items = value.indices.copy().tolist()
            train_dict[idx] = items
...
def kge_score(h, r, t, name="kge_score"):
    with tf.name_scope(name=name):
        return tf.reduce_sum(tf.square((h + r - t)), 1, keepdims=True)
...
def get_info(result: dict, epoch=None, auc=None, f1=None, test=True):
    info = f"epoch {epoch}\n"
    
    for metric, values in result.items():
        values = '\t'.join(map(lambda x: str(format(x, '.4f')), values))
        metric = metric.ljust(10, ' ')
        info += f'{metric}\t{values}\n'
