...
class UniEvaluator(CPPEvaluator):
    ...
    * **Second**, this class and its evaluation metrics can automatically fit
    ...
    def evaluate(self, model, test_users=None, pos=False):
        """Evaluate `model`.
        ...
        batch_result = []
        # positions = [0] * 11  # 0-100, 100-200, 200-300, 300-400, 400-500, 500-600,600-700,700-800,800-900,900-1000,1000-
        # limits = len(positions) - 1
        for batch_users in test_users:
            ...
            # 有了rankings和test_item，我想知道序号
            # def _get_positions(sorted_item_matrix, pos_items, thread_num):
            #     def _cal_one_user(idx):
            #         items = sorted_item_matrix[idx]
            #         pos_item = pos_items[idx]
            #
            #         # get the idx of each test item
            #         for item in pos_item:
            #             pos = items.index(item) // 100
            #             if pos > limits:
            #                 positions[-1] += 1
            #             else:
            #                 positions[pos] += 1
            ...
            #         executor.map(_cal_one_user, range(len(test_items)))
            # if pos:
            #     sorted_items = np.argsort(-ranking_score, axis=1).tolist()  # 按行排列, 得到按分值降序排列后的索引(items）
            #     _get_positions(sorted_items, test_items, thread_num=self.num_thread)
        ...
        buf = dict()
        for idx, metric_idx in enumerate(self.metrics):
            buf[re_metric_dict[metric_idx]] = final_result[idx]
